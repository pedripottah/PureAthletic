import { selectYouthFootballRoutine } from "./youth-football-catalog.mjs";
...
function createRestDay(date, reasonCodes = ["recovery_spacing"]) {
  return {
    date,
    dayType: "rest",
...
function createFixedDay(date, commitment) {
  return {
    date,
    dayType: `fixed_${commitment.type}`,
...
export function buildCanonicalPlan(rawInput, data) {
  ...
  const commitmentByDate = new Map(commitments.map((commitment) => [commitment.date, commitment]));
  const matchDates = commitments.filter((commitment) => commitment.type === "match").map((commitment) => commitment.date);
  const days = weekDates.map((date) => commitmentByDate.has(date)
    ? createFixedDay(date, commitmentByDate.get(date))
    : createRestDay(date));
  ...
  if (rawInput.guardianApproval !== true) {
    return {
      ...
      days: days.map((day) => day.dayType === "rest"
        ? createSafetyDay(day.date, ["guardian_approval_missing"], "Adult approval required", "A parent or guardian must approve participation before training is planned.")
        : day),
  ...
  if (painLevel !== "none") {
    return {
      planVersion: PLAN_VERSION,
      athleteContext: { ageGroup: rawInput.ageGroup, experience: rawInput.experience, goal: rawInput.goal },
      days: days.map((day) => day.dayType === "rest" ? createSafetyDay(day.date, ["pain_reported"], "Pause training and tell an adult", "Reported pain requires responsible-adult review before automated training continues.") : day),
  ...
  for (const date of weekDates) {
    ...
    if (nearestMatchDays * 24 < routine.matchBufferHours) {
  ...
  if (!scheduled) {
    for (const day of days) {
      if (day.dayType === "rest") day.reasonCodes = ["no_safe_candidate", "recovery_spacing"];
    }
  }
  ...
  return {
    planVersion: PLAN_VERSION,
    athleteContext: {
      ageGroup: selection.recommendation.ageGroup,
      ageBandId: selection.recommendation.ageBandId,
      experience: selection.recommendation.experience,
      goal: selection.recommendation.goalId
    },
    days,
    decisionTrace: {
